...
function loadConfig() {
	...
	$id('shader-1').value = config.scaleMode;
}
loadConfig();

function uiSaveConfig() {
	var newScaleMode = parseInt($id('shader-1').value) || 0;
...
function wasmReady() {
	...
	isWasmReady = true;
	document.getElementById('wasm-loading').classList.add('disable')
}
...
function loadRomArrayBuffer(arrayBuffer) {
	...
	document.getElementById('welcome').classList.add('disable');
	document.getElementById('vk-layer').classList.remove('disable'); 	 	
...
function handleTouch(event) {
	...
	if (keyState['menu'][2]) {
		var pauseMenu = document.getElementById('menu-list-pad');
		var menuPad = document.getElementById('menu-pad');
			pauseMenu.classList.toggle('inactive');
...
document.onkeyup = function(e) {
	...
	if (k >= 0) {
	    ...
	    if (e.keyCode == 27) {
	        document.getElementById('pause-menu').classList.remove('disable')
	    }
};
...
function showMsg(msg) {
	...
	document.getElementById('message').classList.add('act');
	messageTimeout = setTimeout(function () {
		document.getElementById('message').classList.remove('act');
	}, 1500);
}
...
(function resetGame() {
	const resetGame = document.getElementById('img-shader');
	let clickCount = 0;
	let lastClickTime = 0;
	
	resetGame.addEventListener('click', handleButtonClick);
...
var setTurbo = document.getElementById('turbo');
var turboState = 0;
setTurbo.addEventListener('click', function() {
    turboState++;
    if (turboState > 2) {
        turboState = 0;
    }
    switch (turboState) {
        case 0:
			turboMode = false;
            turboFast = false;
            this.classList.remove('turbo-medium');
			this.classList.remove('turbo-fast');
			showMsg('default.');
            break;
        case 1:
			turboMode = true;
            turboFast = false;
            this.classList.add('turbo-medium');
            this.classList.remove('turbo-fast');
			showMsg('medium.');
            break;
        case 2:
...
const shader = document.getElementById('shader');
		...
		increaseButton.addEventListener('click', function() {
			...
			imgshader.style.opacity = opacity;
			showMsg( 'Opacity ' + opacity.toFixed(2));
		});
		...
		var setLayoutButton = document.getElementById('set-layout');
		var Ids = ['set-layout', 'dpad-r', 'dpad-l', 'canvas', 'message-div', 'img-shader'];
		var isScaled = false;
		setLayoutButton.addEventListener('click', function() {
			document.getElementById('gba-canvas').classList.toggle('not-scale');
			document.getElementById('gba-canvas').classList.toggle('scale');
			Ids.forEach(id => {
				var element = document.getElementById(id);
				if (element) {
					if (element.classList.toggle('scale')) {
						showMsg('zoom screen.'); 
					} else {
						showMsg('default screen.'); 
					}
				}
			});
			var gbaCanvas = document.getElementById('gba-canvas');
			function scaleCanvas() {
			if (gbaCanvas.classList.contains('scale')) {
				const screenWidth = window.innerWidth;
				const scale = (screenWidth - 30) / 1920;
				gbaCanvas.style.transform = `scale(${scale})`;
				isScaled = true;
			} else {
				if (isScaled) {
					gbaCanvas.style.transform = null;
					isScaled = false;
				}
			}
		}
		...
		document.getElementById('set-cheat').addEventListener('click', function() {
			document.getElementById('cheat-dialog').classList.toggle('active')
			});
		document.getElementById('back-dialog').addEventListener('click', function() {
			document.getElementById('cheat-dialog').classList.toggle('active')
			});